...
knownProcessOptions = frozenset((
    'cmds',  # both process and subplot
...
keysAcceptingIterable = frozenset(
    ('cmds', 'set', 'unset', 'equation', 'equation_below', 'equation_above'))
...
def _getGnuplotFeatures():
    ...
    try:
        out = subprocess.check_output(
            (gnuplot_executable, '-e', "set view equal"),
            stderr=subprocess.STDOUT,
            env=env).decode()
...
def _massageProcessOptionsAndGetCmds(processOptions):
    ...
    if processOptions.get('hardcopy') is not None:
        if processOptions.get('output') is not None:
            raise GnuplotlibError(
                "Pass in at most ONE of 'hardcopy' and 'output'")
    ...
    if processOptions.get('output') is not None and \
       processOptions.get('terminal') is None:
        ...
        if not m:
            raise GnuplotlibError(
                "Only .eps, .ps, .pdf, .png, .svg and .gp output filenames are supported if no 'terminal' plot option is given"
            )
        ...
        terminalOpts = {
            ...
            'png':
            'pngcairo noenhanced size 1024,768 transparent crop font ",12"',
    ...
    if processOptions.get('terminal') is not None:
        if is_knownInteractiveTerminal(processOptions['terminal']):
            ...
            if processOptions.get('output', '') != '':
                sys.stderr.write(
                    "Warning: requested a known-interactive gnuplot terminal AND an output file. Is this REALLY what you want?\n"
                )
...
def _massageSubplotOptionsAndGetCmds(subplotOptions):
    ...
    for option in subplotOptions:
        if not option in knownSubplotOptions:
            raise GnuplotlibError(
                '"{}" is not a valid subplot option'.format(option))
    ...
    if Nsquare > 1:
        raise GnuplotlibError(
            "At most 1 'square...' option could be enabled. Instead I got {}".
            format(Nsquare))
    ...
    if subplotOptions.get('3d'):
        ...
        if not 'equal_3d' in features and \
           ( subplotOptions.get('square_xy') or subplotOptions.get('square') ):
            ...
            sys.stderr.write(
                "Your gnuplot doesn't support square aspect ratios for 3D plots, so I'm ignoring that\n"
            )
    ...
    for axis in ('x', 'x2', 'y', 'y2', 'z', 'cb'):
        if axis + 'label' in subplotOptions:
            cmds.append('set {axis}label "{label}"'.format(
                axis=axis, label=subplotOptions[axis + 'label']))
    ...
    for axis in ('x', 'x2', 'y', 'y2', 'z', 'cb'):
        ...
        if (opt_min is not None
                or opt_max is not None) and opt_range is not None:
            raise GnuplotlibError(
                "{0}min/{0}max and {0}range are mutually exclusive".format(
                    axis))
        ...
        if opt_range is not None:
            if not isinstance(opt_range, (list, tuple)):
                opt_range = [
                    ...
                    for x in opt_range.split(':')
                ]
            if len(opt_range) != 2:
                raise GnuplotlibError(
                    '{}range should have exactly 2 elements'.format(axis))
        ...
        if axis in have_set_link:
            if axis  in ranges and not ranges_default[axis ] and \
               axis2 in ranges and not ranges_default[axis2]:
                raise GnuplotlibError(
                    f"We're linking {axis},{axis2} so the range of at most one of these may be given"
                )
...
class gnuplotlib:
    ...
    def __init__(self, **plotOptions):
        ...
        self.processOptionsCmds = _massageProcessOptionsAndGetCmds(
            self.processOptions)
    ...
    def _safelyWriteToPipe(self, input, flags=''):
        ...
        def barfOnDisallowedCommands(line):
            ...
            if re.match(
                    r'''(?: .*;)?       # optionally wait for a semicolon
                            \s*
                            set\s+print\b''', line, re.X):
                raise GnuplotlibError(
                    "Please don't 'set print' since I use gnuplot's STDERR for error detection"
                )
            ...
            if re.match(
                    r'''(?: .*;)?       # optionally wait for a semicolon
                            \s*
                             print\b''', line, re.X):
                raise GnuplotlibError(
                    "Please don't ask gnuplot to 'print' anything since this can confuse my error detection"
                )

            if re.match(
                    r'''(?: .*;)?       # optionally wait for a semicolon
                            \s*
                            set\s+terminal\b''', line,
                    re.X) and flags != 'terminal':
                raise GnuplotlibError(
                    "Please do not 'set terminal' manually. Use the 'terminal' plot option instead"
                )
            ...
            if re.match(
                    r'''(?: .*;)?       # optionally wait for a semicolon
                            \s*
                            set\s+output\b''', line,
                    re.X) and not re.match('output', flags):
                raise GnuplotlibError(
                    "Please do not 'set output' manually. Use the 'output' plot option instead"
                )
        ...
        while not fromerr.endswith(checkpoint):
            ...
            if len(byte) == 0:
                ...
                if returncode is not None:
                    # Yep. It died.
                    raise Exception(
                        f"gnuplot child died. returncode = {returncode}")
            ...
            fromerr += byte
            self._logEvent(
                "Read byte '{}' ({}) from gnuplot child process".format(
                    byte, hex(ord(byte))))
    ...
    def _checkpoint(self, flags=''):
        ...
        ignore_known_test_failures = re.search(r'ignore_known_test_failures',
                                               flags)
        ...
        if self.processOptions.get(
                'notest'
        ) and not waitforever and not final and not printwarnings:
            return None, None
        ...
        fromerr = self._receive_until_checkpoint_or_timeout(
            checkpoint, waitforever)

        m = re.search(rf'\s*(.*?)\s*{checkpoint}$', fromerr, re.M + re.S)
        if m is None:
            raise Exception(
                f"checkpoint '{checkpoint}' not found in received string '{fromerr}'"
            )
        fromerr = m.group(1)

        warningre = re.compile(r'^\s*(.*?(?:warning|undefined).*?)\s*$',
                               re.M + re.I)
        ...
        if ignore_known_test_failures:
            ...
            r = re.compile(
                r'''^gnuplot>\s*plot.*$                        # the test plot command
                               \n^\s+\^\s*$                               # ^ mark pointing to where the error happened
            ...
            r = re.compile(
                r'^.*_min should not equal .*_max!.*$',  # actual 'min != max' complaint
                re.M)
            fromerr = r.sub('', fromerr)

            # Labels or titles that are too long can complain about stuff being
            # too small to hold plot
            r = re.compile(r'''^.*too small to hold plot.*$''', re.M)
            fromerr = r.sub('', fromerr)
            r = re.compile(r'''^.*Check plot boundary.*$''', re.M)
            fromerr = r.sub('', fromerr)
            ...
            r = re.compile(r'^.*Image grid must be at least.*$', re.X + re.M)
            fromerr = r.sub('', fromerr)
    ...
    def _logEvent(self, event):
        ...
        print("==== PID {} at t={:.4f}: {}".format(
            self.gnuplotProcess.pid if self.gnuplotProcess else '(none)', t,
            event),
              file=sys.stderr)
    ...
    def _sendCurve(self, curve):
        ...
        if self._plotCurveInASCII(curve):
            ...
            if curve.get('matrix'):
                np.savetxt(
                    pipe,
                    nps.glue(*curve['_data'], axis=-2).astype(np.float64,
                                                              copy=False),
                    '%s')
...
labels with spaces in them
                    ...
                    try:
                        is_string = type(e) is np.string_
                    except:
                        is_string = False
                    try:
                        is_bytes = type(e) is np.bytes_
                    except:
                        is_bytes = False
            ...
            nps.mv(nps.cat(*curve['_data']), 0,
                   -1).astype(np.float64, copy=False).tofile(pipe)
    ...
    def _getPlotCmd(self, curves, subplotOptions):
        ...
        def binaryFormatcmd(curve):
            ...
            fmt = ''
            if curve.get('matrix'):
                fmt += 'binary array=({},{})'.format(
                    curve['_data'][0].shape[-1], curve['_data'][0].shape[-2])
        ...
        plotCurveCmds = []
        plotCurveCmdsMinimal = [
        ]  # same as above, but with a single data point per plot only
        ...
        if 'rgbimage' in subplotOptions:
            if not os.access     (subplotOptions['rgbimage'], os.R_OK) or \
               not os.path.isfile(subplotOptions['rgbimage']):
                raise GnuplotlibError(
                    "Requested image '{}' is not a readable file".format(
                        subplotOptions['rgbimage']))

            plotCurveCmdsNonDataBefore.append(
                '"{0}" binary filetype=auto flipy with rgbimage title "{0}"'.
                format(subplotOptions['rgbimage']))
        ...
        for curve in curves:
            ...
            if not self._plotCurveInASCII(curve):
                ...
                plotCurveCmds.append(
                    f"'{plot_pipe_name}' {formatFull}    {optioncmds}")
                plotCurveCmdsMinimal.append(
                    f"'{plot_pipe_name}' {formatMinimal} {optioncmds}")
                ...
                if using is None:
                    using = ':'.join(
                        str(x + 1) for x in range(curve['tuplesize']))
                using = ' using ' + using
                ...
                plotCurveCmds.append(
                    f"'{plot_pipe_name}' {matrix} {using} {optioncmds}")
                plotCurveCmdsMinimal.append(
                    plotCurveCmds[-1])  # same testing command

                testData_curve = ''
                if curve.get('matrix'):
                    testmatrix = "{0} {0}\n" + "{0} {0}\n" + "\ne\n"
                    testData_curve = testmatrix.format(testdataunit_ascii) * (
                        curve['tuplesize'] - 2)
                else:
        ...
        cmd = basecmd + ','.join(plotCurveCmdsNonDataBefore + plotCurveCmds +
                                 plotCurveCmdsNonDataAfter)
        cmdMinimal = basecmd + ','.join(plotCurveCmdsNonDataBefore +
                                        plotCurveCmdsMinimal +
                                        plotCurveCmdsNonDataAfter)
    ...
    def _massageAndValidateArgs(self, curves, curveOptions_base,
        ...
        if len(curves):
            curves = [
                np.array((c, )) if isinstance(c, numbers.Real) else c
                for c in curves
            ]
            if all(isinstance(curve, np.ndarray) for curve in curves):
                ...
                raise GnuplotlibError(
                    "all data arguments should be of type ndarray (one curve) or tuples"
                )
        ...
        def reformat(curve):
            ...
            for x in curve:
                if x.size <= 0:
                    ...
                    raise GnuplotlibError(
                        "Received data where SOME (but not ALL) of the arrays had length-0. Giving up"
                    )

            if 'tuplesize' in d and d['tuplesize'] < 0:
                if len(curve) != 1:
                    raise GnuplotlibError(
                        "tuplesize<0 means that only a single numpy array of data should be given: all data is in this array"
                    )
                d['tuplesize'] = -d['tuplesize']
                d['_data'] = list(nps.mv(nps.atleast_dims(curve[0], -2), -1,
                                         0))
            else:
                d['_data'] = list(curve)
            return d
        ...
        for curve in curves:
            ...
            for opt in curve:
                ...
                if not opt in knownCurveOptions:
                    raise GnuplotlibError(
                        "'{}' not a known curve option".format(opt))

            axes = curve.get('axes')
            if axes is not None:
                if not axes in knownAxes:
                    raise GnuplotlibError(
                        f'"axes" must be one of {knownAxes}, but got {axes=}')
            if curve.get('y2'):
                if axes is not None:
                    raise GnuplotlibError(
                        '"y2" and "axes" are mutually exclusive')
                del curve['y2']
                curve['axes'] = 'x1y2'
            ...
            if curve.get('histogram'):
                ...
                if 'tuplesize' in curve and curve['tuplesize'] != 1:
                    raise GnuplotlibError(
                        "histograms only make sense with tuplesize=1. I'll assume this if you don't specify a tuplesize"
                    )
                curve['tuplesize'] = 1

                if 'using' in curve:
                    raise GnuplotlibError(
                        "'using' cannot be given with 'histogram'. I'll make up my own 'using' in this case"
                    )
                ...
                if 'with' not in curve:
                    if re.match(r'freq|fnorm',
                                histogram_type) and 'with' not in curve:
                ...
                if 'binwidth' in curve:
                    raise GnuplotlibError(
                        "'binwidth' only makes sense with 'histogram'")
            ...
            if Ndata < curve['tuplesize']:
                ...
                if Ndata + 1 == curve['tuplesize']:
                    ...
                    # domain 0,1,2,...
                    curve['_data'].insert(
                        0, np.arange(curve['_data'][0].shape[-1]))
            ...
            for datum in curve['_data']:
                ...
                if curve.get('matrix') and datum.ndim < 2:
                    raise GnuplotlibError(
                        "Tried to plot against an implicit 2D domain, but was given less than 2D data"
                    )
        ...
        for curve in curves:
            ...
            prototype_onearg = tuple('n{}'.format(i)
                                     for i in range(ndims_input))
            prototype = (prototype_onearg, ) * len(curve['_data'])

            # grab all option keys that have numpy arrays as values. I broadcast
            # these as well
            np_options_keys = [
                k for k in curve.keys() if isinstance(curve[k], np.ndarray)
            ]
            N_options_keys = len(np_options_keys)
            prototype_np_options = ((), ) * N_options_keys

            for args in nps.broadcast_generate(
                    prototype + prototype_np_options,
                ...
                curve_slice[
                    '_data'] = args[:-N_options_keys] if N_options_keys else args

                for ikey in range(N_options_keys):
                    curve_slice[np_options_keys[ikey]] = args[-N_options_keys +
                                                              ikey]
    ...
    def plot(self, *curves, **jointOptions):
        ...
        def test_plot(testcmd, testdata):
            ...
            checkpointMessage, warnings = self._checkpoint(
                'ignore_known_test_failures')
        ...
        def plot_process_header():
            ...
            if self.processOptions.get('terminal') == 'gp':
                ...
                if 'terminal' in self.processOptions:
                    self._safelyWriteToPipe(
                        "set terminal " + self.processOptions['terminal'],
                        'terminal')
                ...
                if 'output' in self.processOptions:
                    if self.processOptions['output'] != '':
                        ...
                        self._safelyWriteToPipe(
                            'set output "' + self.processOptions['output'] +
                            '"', 'output')
                    else:
                       ...
                       is_knownInteractiveTerminal(self.processOptions['terminal']):
                        ...
                        if not _data_dump_only(self.processOptions):
                            if self.fdDupSTDOUT is None:
                                raise GnuplotlibError(
                                    "I need to plot to STDOUT, but STDOUT wasn't available"
                                )
                            self.processOptions['output'] = '/dev/fd/' + str(
                                self.fdDupSTDOUT)
                        else:
        ...
        def ingest_joint_options(jointOptions, subplotOptions_base,
                                 curveOptions_base):
        ...
        def make_subplot_data(subplotOptions_base, curveOptions_base, *curves,
                              **jointOptions):
            ...
            subplotOptionsCmds = _massageSubplotOptionsAndGetCmds(
                subplotOptions)

            curves = self._massageAndValidateArgs(curves, curveOptions,
                                                  subplotOptions)
        ...
        if not is_multiplot:
            ...
            def make_subplot_data_embedded_kwargs(subplot):
                ...
                return make_subplot_data(subplotOptions_base,
                                         curveOptions_base, *subplot, **d)

            subplots = [
                make_subplot_data_embedded_kwargs(subplot)
        ...
        if not self.processOptions.get('notest'):
            ...
            self._safelyWriteToPipe("set terminal pop; set terminal push",
                                    'terminal')
...
def wait(*args):
    ...
    if len(args) == 0:
        ...
        raise Exception(
            "gnuplotlib.wait() takes an inline list of plots or a single list-of-plots argumnent. Got neither"
        )
...
def add_plot_option(d, key=None, values=None, overwrite=None, **kwargs):
...
- overwrite: optional boolean that controls how we handle overwriting keys that
  ...
  the previous values. elif not overwrite: we leave the previous value
    ...
    if kwargs:
        add_plot_option(d, key, values, overwrite)
        for key in kwargs:
            add_plot_option(d, key, kwargs[key], overwrite)
        return
    ...
    if key_normalized not in keysAcceptingIterable:
        if len(values) > 1:
            raise GnuplotlibError(
                "plot options given multiple values for key '{}'".format(
                    key_normalized))
        if key in d or key_normalized in d:
            ...
            if (overwrite is not None) and overwrite:
                ...
                raise GnuplotlibError(
                    "plot options already have a value for key '{}'. Pass 'overwrite=False' to use the existing one of 'overwrite=True' to use the new one"
                    .format(key_normalized))
